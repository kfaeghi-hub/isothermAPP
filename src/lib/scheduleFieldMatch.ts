...
import { alternatesFor, convertValue, type Conversion } from './unitConvert'
...
export function matchScheduleSpec(
  ...
  const first = Object.entries(spec).map(([k, v]) => matchScheduleField(k, String(v), declared))

  const claims = new Map<string, number>()
  for (const m of first) {
    if (m.field && (m.kind === 'exact' || m.kind === 'converted')) {
