...
| **The family shipped 36 templates that were one template** | 113 templates, one per master | Phase 1 seeded one template per master **file**, and the corpus repeats masters across system folders — `S02-Pump P- CSP.doc` appears in twelve. Names came from the master's `SUBJECT:` row, which on plumbing masters names the **system**. Six names, one checklist. **Found by owner-scroll of the catalog** — every gate the campaign ran was per-template, and each of the six was individually correct. The fix is the [naming law](../ARCHITECTURE.md#template-naming-law--the-register-names-the-thing-not-the-source-document); the gate it argues for is **render-and-look at CATALOG level**, not only at document level. A per-item check cannot see a duplicated item. |
