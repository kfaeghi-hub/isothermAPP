...
| R18 | **Equipment keys are ruled, never invented** — ahu (incl. RTU, Direct-Fired MAU), pump (incl. sump, DHW circulation), fan (incl. fume exhausters), fcu (incl. split-system AC, packaged A/C variants), heat_pump, chiller, cooling_tower (incl. fluid cooler), boiler (FD/ND/steam), erv (heat recovery wheel), generator (Word sweep), vav (VAV terminal units incl. fan-powered variants — minted 2026-07-21; CAV stays null), ats (connected 2026-07-21 — field-def set pre-existed on the register). Everything else → null → basic fallback nameplate (works; verified). | Step-1 + Word sweep + PFC rulings |
...
| R22 | **Standard marks are data** — certification/standard acronyms appearing as field CONTENT (CSA/UL listing marks on a nameplate, code references in a check) are recorded as grid-cell/comment values, never in labels — the branding sweep stays absolute with zero exceptions. Record the option set in _extraction. Requirement references in check labels ("Conforms to SMACNA") are content and stay. | Synchronous Motor + Word pilot |
...
- **check_table render mode** (built at the 2.6.11.7 gate, fleet-proven at 24 units): `checklist_templates.render_mode='check_table'` renders transposed fleet records — landscape, units as rows, items as numbered chunked columns (tag column repeated per chunk), procedures-key legend from item hints, completed cells = status + date (N red), DOCX attempted-but-optional. Fleet checklists in future campaigns (Start-Up) opt in via the flag; extraction stays the standard item model.
- **IEL grammar** (Batch F, Isotherm's own Elec-IEL masters): title -> name; single preamble skip range (register + Submittal/Approvals + signature rows + Information blocks + Associated Checklists); "Requested documentation submitted | Rec'd | Comments" header opens that section; "Check if Acceptable | NA | Comment" headers skip; config-declared bank/sub-bank banners; wrapped item labels merge upward on the lowercase-start rule (declared merged_rows); "Commissioning Authority" as printed. Extracted from **PDF render twins** (dump-pdf.mjs + harness PDF mode with furniture auto-skips) while Word COM is blocked — the .doc masters stay authoritative; if COM recovers, future work MAY re-verify against them but the twins are content-identical.
- **PDF-mode caveat**: blank fill-in table rows are invisible in the PDF text layer — schedule-grid row capacities are declared assumptions, not counts (12 for IEL calibration).
- **Word COM environment flag (2026-07-21) — CLEARED 2026-08-05**: Documents.Open hung machine-wide (even on previously convertible files; ShareSync Office Plugin implicated, disabling insufficient). Fixed on Tony's desktop; re-verified independently by `probe-word-com.ps1` — **2/2 converted**, including `ats_checklist.doc`, the exact file the 2026-07-21 probe hung on, plus one CSA `.doc` that had converted before it (one of each, deliberately: a probe that only tries the known-bad file cannot tell *fixed* from *that file was special*). The probe bounds every COM call in a job with a hard timeout and kills only the WINWORD processes it started, so a regression is reported as FAIL rather than waited on. It runs only against working copies already in gitignored `samples/` — Word writes an owner-lock file next to whatever it opens, so a "read-only" probe against ShareSync would still be a write to it. **The IEL PDF-render-twin path stays valid and stays used where it is already proven; nothing is re-extracted just because COM works again.**
