...
//
// The mapping, by ROLE — read out of the styles themselves, not guessed:
//   443C8F  is always w:fill on a heading whose w:color is FFFFFF  → a band
//           carrying white text. That is DOC.BAND.       → 000000
//   5D55AF  is always w:color — the second-level heading text
//           (CompanyName-ABC, ListHeading2Char).         → 4D4D4D  (DOC.BAND_UNIT)
//
...
const MAP = { '443C8F': '000000', '5D55AF': '4D4D4D' }
const RETIRED = ['443C8F', '5D55AF', '7F78CB', 'E3E1F5', 'CFCCE0', 'E1DEEB', 'F7F6FC',
                 '1F3A5F', 'E8432D', 'D9E2F3', 'F4F7FB']
...
let xml = parts[PART].toString('utf8')
const counts = {}
for (const [from, to] of Object.entries(MAP)) {
  const re = new RegExp(from, 'gi')
  counts[from] = (xml.match(re) ?? []).length
  xml = xml.replace(re, to)
}
console.log('replacements:', Object.entries(counts).map(([k, v]) => `${k}→${MAP[k]} ×${v}`).join(', '))
if (Object.values(counts).every(n => n === 0)) {
  console.log('skeleton already clean — nothing to do'); process.exit(0)
}

// The assertion that makes this reviewable: nothing outside styles.xml moved,
// and inside styles.xml only the mapped hexes did.
// Both sides of every mapping are 6 hex characters, so a correct substitution
// cannot change the part's length. A length change means something else moved.
const check = parts[PART].toString('utf8')
if (xml.length !== check.length) { console.error('REFUSE: styles.xml length changed'); process.exit(1) }
for (const h of RETIRED) {
  if (new RegExp(h, 'i').test(xml)) { console.error(`REFUSE: ${h} survives in styles.xml`); process.exit(1) }
}

const zipOut = new JSZip()
for (const [name, buf] of Object.entries(parts))
  zipOut.file(name, name === PART ? Buffer.from(xml, 'utf8') : buf)
...
console.log(`parts: ${names.length} · unchanged outside ${PART}: yes · residual retired hex: ${residual.length ? residual.join(' ') : 'none'}`)
